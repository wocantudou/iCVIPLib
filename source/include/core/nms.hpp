  ...
  public:
    ...
    inline int nms(const std::vector<cv::Rect2f> &src_rects,
        ...
        while (idxs.size() > 0) {
            ...
            for (auto pos = std::begin(idxs); pos != std::end(idxs);) {
                ...
                float union_area =
                    main_rect.area() + query_rect.area() - int_area;
    ...
    inline int nms2(const std::vector<cv::Rect2f> &src_rects,
        ...
        if (size > max_init_nms_cnt) {
            ...
            int del_idx = 0;
            // for (std::multimap<float, size_t>::iterator iter = idxs.begin();
            // iter
            // != idxs.end(); )
...
#define iCVNMSClass ICV_SAFE_OPENCV::iCVNMS
#define iCVNMSInst ICVSingletonClass<iCVNMSClass>::get_inst